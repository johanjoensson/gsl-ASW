...
double Structure_constant::operator()(GSL::Vector::Const_View r) const
{
	...
	Hankel_function H(lm {0, 0, 0});
	for (int lpp = 0; lpp <= l_int; lpp++){
		H.set_l(lm{0, lpp, 0});
		m_sum = GSL::Result();
		for (int mpp = -lpp; mpp <= lpp; mpp++){
			a = real_gaunt(l2, l1, lm {0, lpp, mpp});
			if (std::abs(a.val) > 1E-15){
			    m_sum += a*cubic_harmonic(lm {0, lpp, mpp}, r/r.norm());
			}
		}
...
double Structure_constant::dot(GSL::Vector::Const_View r) const
{
	double k_fac = GSL::pow_int(kappa, l1.l + l2.l);
	...
	for (int lpp = 0; lpp <= l_int; lpp++){
		...
		for (int mpp = -lpp; mpp <= lpp; mpp++){
			a = real_gaunt(l2, l1, lm {0, lpp, mpp});
			if (std::abs(a.val) > 1E-15){
			    m_sum += a*cubic_harmonic(lm {0, lpp, mpp}, r/r);
			}
		}

		tmp.val = (l1.l + l2.l - lpp)/(-kappa*kappa)*H(kappa*r_norm);
		H.set_l(lm {0, lpp - 1, 0});
...
void Bloch_summed_structure_constant::Container::add(const Crystal_t<3, Atom>& c,
	const lm& lint, const lm& lp, const lm& l, const double kappa,
	GSL::Vector::Const_View tau, GSL::Vector::Const_View kp)
{
	Bloch_summed_structure_constant B;
	values_m.insert({Bloch_summed_structure_constant::Key(l, lp, kappa, tau[0], tau[1], tau[2], kp[0], kp[1], kp[2]), B.calc(c, lint, lp, l, kappa, tau, kp, D_m)});
	dot_values_m.insert({Bloch_summed_structure_constant::Key(l, lp, kappa, tau[0], tau[1], tau[2], kp[0], kp[1], kp[2]), B.dot(c, lint, lp, l, kappa, tau, kp, D_m)});
}

GSL::Complex Bloch_summed_structure_constant::Container::get(const Crystal_t<3, Atom>& c, const lm& lint, const lm& lp, const lm& l, const double kappa, GSL::Vector::Const_View tau,	GSL::Vector::Const_View kp)
{

	// Check if value has already been calculated
	auto it = values_m.find(Bloch_summed_structure_constant::Key(lp, l, kappa, tau[0], tau[1], tau[2], kp[0], kp[1], kp[2]));
	if(it != values_m.end()){
		return it->second;
	}
	it = values_m.find(Bloch_summed_structure_constant::Key(l, lp, kappa, tau[0], tau[1], tau[2], kp[0], kp[1], kp[2]));
	if(it != values_m.end()){
		return (l.l - lp.l) % 2 == 0 ? it->second : -it->second;
	}
	it = values_m.find(Bloch_summed_structure_constant::Key(lp, l, kappa, -tau[0], -tau[1], -tau[2], -kp[0],  -kp[1], -kp[2]));
	if(it != values_m.end()){
		return (l.l - lp.l) % 2 == 0 ? it->second : -it->second;
	}
	it = values_m.find(Bloch_summed_structure_constant::Key(lp, l, kappa, tau[0], tau[1], tau[2], -kp[0], -kp[1], -kp[2]));
	...
	GSL::Vector::Const_View tau,	GSL::Vector::Const_View kp)
{
	...
	// Check if value has already been calculated
	auto it = dot_values_m.find(Bloch_summed_structure_constant::Key(lp, l, kappa, tau[0], tau[1], tau[2], kp[0], kp[1], kp[2]));
	if(it != dot_values_m.end()){
		return it->second;
	}
	it = dot_values_m.find(Bloch_summed_structure_constant::Key(l, lp, kappa, tau[0], tau[1], tau[2], kp[0], kp[1], kp[2]));
	if(it != dot_values_m.end()){
		return (l.l - lp.l) % 2 == 0 ? it->second : -it->second;
	}
	it = dot_values_m.find(Bloch_summed_structure_constant::Key(lp, l, kappa, -tau[0], -tau[1], -tau[2], -kp[0], -kp[1], -kp[2]));
	if(it != dot_values_m.end()){
		return (l.l - lp.l) % 2 == 0 ? it->second : -it->second;
	}
	it = dot_values_m.find(Bloch_summed_structure_constant::Key(lp, l, kappa, tau[0], tau[1], tau[2], -kp[0], -kp[1], -kp[2]));
...
void Bloch_summed_structure_constant::Container::recalculate_all(const Crystal_t<3, Atom>& c, const lm& lint)
{
    ...
    for(const auto& it : old_values){
		...
		tau[0] = std::get<3>(it.first);
		tau[1] = std::get<4>(it.first);
		tau[2] = std::get<5>(it.first);
     	kp[0] = std::get<6>(it.first);
		kp[1] = std::get<7>(it.first);
		kp[2] = std::get<8>(it.first);
        this->add(c, lint, lp, l, kappa, tau, kp);
    }
...
GSL::Complex Bloch_summed_structure_constant::calc(
	...
	GSL::Vector::Const_View tau, GSL::Vector::Const_View kp) const
{
...
GSL::Complex Bloch_summed_structure_constant::calc(
	const Crystal_t<3, Atom>& c, const lm& lint, const lm& lp, const lm& l, const double kappa,
	GSL::Vector::Const_View tau, GSL::Vector::Const_View kp, Bloch_sum::Container& Ds) const
{
	GSL::Complex sum(0., 0.);
	double k_fac = 1;
	if(std::abs(kappa) > 1e-10){
		...
		for (int lpp = 0; lpp <= lint.l; lpp++){
			for (int mpp = -lpp; mpp <= lpp; mpp++){
				GSL::Result a = real_gaunt(l, lp, lm {0, lpp, mpp});
				if (std::abs(a.val) > 1E-16){
				    sum += GSL::pow_int(-1, lpp)*GSL::pow_int(kappa, -lpp)*
					a.val*Ds(lm {0, lpp, mpp}, kappa, c, tau, kp);
				}
			}
		}
	}else if (l.l + lp.l  <= lint.l){
		int lpp = l.l + lp.l;
		for (int mpp = -lpp; mpp <= lpp; mpp++){
			GSL::Result a = real_gaunt(l, lp, lm {0, lpp, mpp});
			if (std::abs(a.val) > 1E-16){
	...
	GSL::Vector::Const_View tau, GSL::Vector::Const_View kp, Bloch_sum::Container& Ds) const
{
	...
	if(std::abs(kappa) > 1e-10){
		...
		for (int lpp = 0; lpp <= lint.l; lpp++){
			for (int mpp = -lpp; mpp <= lpp; mpp++){
				a = real_gaunt(l, lp, lm {0, lpp, mpp});
				if (std::abs(a.val) > 1E-16){
					sum += a.val*
						(2*Ds.dot(lm {0, lpp,	 mpp}, kappa, c, tau, kp) -
						 (l.l + lp.l - lpp)/GSL::pow_int(kappa, 2)*
						 Ds(lm {0, lpp, mpp}, kappa, c, tau, kp))*
		...
		for (int mpp = -lpp; mpp <= lpp; mpp++){
			a = real_gaunt(l, lp, lm {0, lpp, mpp});
			if (std::abs(a.val) > 1E-16){
				sum += a.val*(2*Ds.dot(lm {0, lpp,	 mpp}, kappa, c, tau, kp));
			}
		}
	}
	int sign = l.l % 2 == 0 ? 1 : -1;
